...
func TestPrimitiveProcessor_determineModel(t *testing.T) {
	...
	schema := &jsonSchema.Definition{Type: jsonSchema.String, Model: jsonSchema.Gpt4}
	task := domain.NewFieldTask("test", schema, nil)

	model := processor.determineModel(task.Definition())
	if model != jsonSchema.Gpt4 {
		t.Errorf("Expected Gpt4, got %v", model)
	}
...
func TestGetDefaultModelForProvider(t *testing.T) {
	...
	if model != jsonSchema.Gpt4Mini {
		t.Errorf("Expected Gpt4Mini for openai, got %v", model)
	}

	// Test with LLM_PROVIDER set to gemini
	os.Setenv("LLM_PROVIDER", "gemini")
	model = getDefaultModelForProvider()
	if model != jsonSchema.GeminiFlash {
		t.Errorf("Expected GeminiFlash for gemini, got %v", model)
	}
	...
	model = getDefaultModelForProvider()
	if model != jsonSchema.Gpt4Mini {
		t.Errorf("Expected Gpt4Mini for API_URL, got %v", model)
	}
}
