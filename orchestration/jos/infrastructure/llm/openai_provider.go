...
import (
	...
	"objectGeneration/orchestration/jobSubmitter"
	"objectGeneration/orchestration/jos/domain"
	byteoperations "objectGeneration/llmManagement/byteOperations"
...
type OpenAIProvider struct {
	...
	defaultModel jsonSchema.ModelType
	client       *gogpt.Client
}

func NewOpenAIProvider() *OpenAIProvider {
	// CRITICAL: Always use OPENAI_API_KEY for OpenAI operations (DALL-E, Whisper, TTS)
	// Do NOT use LLM_API_KEY which might be a Gemini key
...
func getDefaultModelForProvider() jsonSchema.ModelType {
	...
	switch provider {
	case "gemini":
		return jsonSchema.GeminiFlash2 // gemini-2.5-flash
	case "openai":
...
func (p *OpenAIProvider) ModelType() jsonSchema.ModelType {
	return p.defaultModel
}

func calculateCost(model jsonSchema.ModelType, usage interface{}) float64 {
...
func (p *OpenAIProvider) GenerateImage(request *domain.ImageGenerationRequest) ([]byte, *domain.ProviderMetadata, error) {
	...
	// Convert string model to jsonSchema.ModelType
	modelType := jsonSchema.ModelType(request.Model)
